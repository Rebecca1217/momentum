...
% 调整参数时间
unit = outerjoin(array2table(tdDate, 'VariableNames', {'Date'}), Cost.unit, ...
    'type', 'left', 'MergeKeys', true, 'Keys', 'Date');
multi = outerjoin(array2table(tdDate, 'VariableNames', {'Date'}), Cost.multi, ...
    'type', 'left', 'MergeKeys', true, 'Keys', 'Date');
...
if num~=0 %num=0的情况：只有一个信号，且该信号到截止时间还未结束或者该信号在最后一行发出
    for i = 1:num %逐个信号计算
        ...
        if clL-opL>1 %不是当根开，下根平的情况
            ...
            %
            opP = (tdDataI(1)+sgn*slip*unit.MiniTick(opL+1))*(1+sgn*fixC); %开仓价
            ...
            for d = 2:clL-opL %逐天计算盈亏
                if tdAdjI(d)~=0 %当日换月，把旧合约全部平掉，然后在新合约上开对应的手数
                    % 平旧合约
                    clOld = sgn*((tdAdjI(d)-sgn*slip*unit.MiniTick(opL+d))*(1-sgn*fixC)-setP)*tdList(opL+d-1,4);
                    % 开新合约
                    opNew = sgn*(ttDataI(d,5)-(tdDataI(d)+sgn*slip*unit.MiniTick(opL+d))*(1+sgn*fixC))*tdList(opL+d,4);
                    pftDly = clOld+opNew;
                else %当日非换月
                    if HandsAdd(d)==0 %如果持仓手数没有变
                        pftDly = sgn*(ttDataI(d,5)-setP)*tdList(opL+d-1,4);
                    elseif HandsAdd(d)>0 %手数增加
                        pftDly = sgn*(ttDataI(d,5)-setP)*tdList(opL+d-1,4)+sgn*(ttDataI(d,5)-(tdDataI(d)+sgn*slip*unit.MiniTick(opL+d))*(1+sgn*fixC))*HandsAdd(d);
                    elseif HandsAdd(d)<0 %手数减少
                        pftDly = sgn*(ttDataI(d,5)-setP)*tdList(opL+d,4)+sgn*((tdDataI(d)-sgn*slip*unit.MiniTick(opL+d))*(1-sgn*fixC)-setP)*abs(HandsAdd(d));
                    end
                end
            ...
            if tdAdjI(end)~=0 %平仓日刚好是换月日,则用旧主力合约的开盘价平，否则就是正常的平仓价平
                clP = (tdAdjI(end)-sgn*slip*unit.MiniTick(clL+1))*(1-sgn*fixC);
            else
                clP = (tdDataI(end)-sgn*slip*unit.MiniTick(clL+1))*(1-sgn*fixC); %合约交易价平              
            end
...
% 乘以合约乘数
tdList(:,5) = tdList(:,5) .* multi.MultiFactor;
